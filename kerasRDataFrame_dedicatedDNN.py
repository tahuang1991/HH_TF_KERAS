...
def loadSignal(treename, masses, variables, cut):
    for mass in masses:
        ...
        w_sig = data_sig.Filter(cut).Define("final_total_weight", weightexpr)
        x_sig = np.vstack([data_sig[var] for var in variables], w_sig["final_total_weight"]).T
